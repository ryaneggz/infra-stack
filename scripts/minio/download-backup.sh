...
[ -d "$destination" ] && [ ! -L "$destination" ] || { echo "Download staging directory is missing or unsafe" >&2; exit 2; }
...
[ "$listed" = "$name" ] && [ -z "${extra:-}" ] || { echo "Unsafe downloaded checksum entry" >&2; exit 1; }
